...
class App extends Component{
  ...
  async componentDidMount() {
    await this.initWeb3()
    // await this.pollData()
    setInterval(this.pollData, 3000); // 1초에 한번
  }
  ...
  // Legacy dapp browsers...
    ...
    // let pot = await this.lotteryContract.methods.getPot().call()  // 컨트랙의 값을 변화 x 블락체인 변화 x 값을 읽기만함.
    // console.log(pot)
  ...
  makeFinalRecords = () => {
    ...
    for(let i=0;i<this.state.betRecords.length;i+=1) {
      if(this.state.winRecords.length > 0 && this.state.betRecords[i].index === this.state.winRecords[w].index){
        ...
        records[i].pot = this.web3.utils.fromWei(this.state.winRecords[w].amount, 'ether'); // pot money 를 eth형식으로 바꿔 records.pot 에 저장
